...
LOGGER_ROOT_DIRECTORY = r'G:\\Python\\PacketLight Automation\\LightWatch_WEB\\Scripts\\SNMP\\LogFiles'
...
def test_snmpv3_device_discovery(page, left_panel: LeftPanel, logger, report):
    ...
    ############################################################
    # Step 6 – Discovery with No Authentication and No Privacy #
    ############################################################
    def step_6():
        refresh_page(page)
        open_device_discovery(left_panel)
    ...
    ####################################
    # Step 8 – Verify LW in SNMP Traps #
    ####################################
    def step_8():
        refresh_page(page)
    ...
    #####################################
    # Step 13 – Verify LW in SNMP Traps #
    #####################################
    def step_13():
        refresh_page(page)
        device_page = page.context.new_page()
    ...
    #####################################
    # Step 18 – Verify LW in SNMP Traps #
    #####################################
    def step_18():
        refresh_page(page)
        device_page = page.context.new_page()
